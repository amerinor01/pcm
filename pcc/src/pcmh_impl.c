...
inline int __flow_signal_get(const void *ctx, size_t user_index) {
    return (
        ((struct flow *)ctx)->datapath_state)[FLOW_SIGNALS_OFFSET + user_index];
}
...
inline void __flow_signal_update(void *ctx, size_t user_index, int val) {
    (((struct flow *)ctx)->datapath_state)[FLOW_SIGNALS_OFFSET + user_index] +=
        val;
}
...
inline void __flow_control_set(void *ctx, size_t user_index, int val) {
    (((struct flow *)ctx)->datapath_state)[FLOW_CONTROLS_OFFSET + user_index] =
        val;
}
