...
AtlahsHtsimApi::AtlahsHtsimApi() = default;
AtlahsHtsimApi::~AtlahsHtsimApi() = default;
...
void AtlahsHtsimApi::Send(const SendEvent &event, graph_node_properties elem) {
    ...
    if (_logsim_interface->get_protocol() == UEC_PROTOCOL) { 
        ...
        UecSrc* uecSrc;
        if (pcm_enable) {
            // std::string tag_ctx_str = std::to_string(tag).substr(0, 1);
            std::string tag_ctx_str = std::to_string(tag/10);
...
void AtlahsHtsimApi::Setup() {
    ...
    if (pcm_enable) {
        std::ifstream p_file(pcm_cc_config_filename);
        if (!p_file.is_open()) {
            std::cerr << "Error: Failed to open PCM config file: " << pcm_cc_config_filename << std::endl;
            exit(0);
        }
        Json::Value p_file_value;
        p_file >> p_file_value;
        for (const auto& tag_ctx_str : p_file_value.getMemberNames()) {
            ctx_to_pcm_cc_params[tag_ctx_str] = p_file_value[tag_ctx_str].asString();
        }
    }
    ...
    for (size_t ix = 0; ix < total_nodes; ix++) {
        ...
        if (pcm_enable) {
            pcm_cc_schedulers.emplace_back(make_unique<pcm_htsim::PcmScheduler>(
                *_eventlist, pcm_handler_delay, pcm_sched_poll_delay, pcm_sched_type_is_async ? pcm_htsim::PcmScheduler::ProgressType::ASYNC : pcm_htsim::PcmScheduler::ProgressType::SYNC));

            for (const auto& [tag_ctx_str, pcm_algo_name] : ctx_to_pcm_cc_params) {
                pcm_cc_schedulers.at(ix)->attachAlgorithm(pcm_algo_name, static_cast<pcm_htsim::PcmScheduler::PcmVmTag>(std::stoul(tag_ctx_str)));
            }
        }
    }
