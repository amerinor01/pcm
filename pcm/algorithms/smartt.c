...
static PCM_FORCE_INLINE bool smartt_quick_adapt(ALGO_CTX_ARGS, pcm_uint t_now,
                                                pcm_uint *cur_cwnd) {
...
static PCM_FORCE_INLINE bool smartt_fast_increase(ALGO_CTX_ARGS,
                                                  pcm_uint num_ecns,
                                                  pcm_uint rtt_sample,
                                                  pcm_uint *cur_cwnd) {
    // printf(
...
static PCM_FORCE_INLINE void smartt_handle_ack(ALGO_CTX_ARGS, pcm_uint num_ecns,
                                               pcm_uint rtt_sample,
                                               pcm_uint t_now,
                                               pcm_uint *cur_cwnd) {
...
int algorithm_main() {
    pcm_uint rtt_sample =
        get_signal(SIG_RTT_SAMPLE); // we don't use/support avg RTT (yet)
    ...
    if (get_signal(SIG_NUM_ACK) > 0) {
        ...
        smartt_handle_ack(ALGO_CTX_PASS, num_ecns, rtt_sample, t_now,
                          &cur_cwnd);
