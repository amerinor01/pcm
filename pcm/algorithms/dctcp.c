...
static PCM_FORCE_INLINE void dctcp_alpha_update(ALGO_CTX_ARGS,
     ...
     */
    if (delivered >= get_local_state_uint(VAR_EPOCH_TO_DELIVER)) {
        // alpha = (1 - g) * alpha + g * F
        pcm_float F = (pcm_float)delivered_ecn / (pcm_float)delivered;
        set_local_state_float(
            VAR_ALPHA, (1 - CONST_GAMMA) * get_local_state_float(VAR_ALPHA) +
                           CONST_GAMMA * F);
        set_local_state_uint(VAR_EPOCH_DELIVERED, 0);
        set_local_state_uint(VAR_EPOCH_ECN_DELIVERED, 0);
        update_signal(SIG_ECN, -get_signal(SIG_ECN));
        /* TODO: support LB path change */
    } else {
        set_local_state_uint(VAR_EPOCH_DELIVERED, delivered);
        set_local_state_uint(VAR_EPOCH_ECN_DELIVERED, delivered_ecn);
    }

    // fprintf("DCTCP: delivered=%llu ecned=%llu to_deliver=%llu alpha=%lf\n",
    //         delivered, delivered_ecn,
    //         get_local_state_uint(VAR_EPOCH_TO_DELIVER),
    //         get_local_state_float(VAR_ALPHA));
}
