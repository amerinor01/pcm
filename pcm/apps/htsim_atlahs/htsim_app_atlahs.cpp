...
int main(int argc, char **argv) {
    ...
    bool pcm_sched_type_is_async = false;
    ...
    pcm_htsim::PcmScheduler::PcmVmTag pcm_lb_algo_dummy_tag =
        std::numeric_limits<pcm_htsim::PcmScheduler::PcmVmTag>::max();
    std::string pcm_cc_config_filename = "";
...
//            cout << "fair_decrease disabled" << endl;
            ...
            pcm_cc_config_filename = argv[i + 1];
            cout << "PCM configuration file: "<< pcm_cc_config_filename << endl;
            i++;
        } else if (!strcmp(argv[i], "-pcm_enable")) {
            pcm_enable = true;
            cout << "PCM enabled " << endl;
    ...
    */
    ...
    for (size_t ix = 0; ix < no_of_nodes; ix++){
        ...
        if (pcm_enable and goal_filename.size() == 0) {
    ...
    if (goal_filename.size() > 0) {
        ...
        if (pcm_enable) api->pcm_cc_config_filename = pcm_cc_config_filename;
        else UecSrc::pcm_cc_config_filename = pcm_cc_config_filename;
        api->pcm_enable = pcm_enable;
        api->pcm_lb_enable = pcm_lb_enable;
        api->ports = ports;
        api->pcm_handler_delay = pcm_handler_delay;
        api->pcm_sched_poll_delay = pcm_sched_poll_delay;
        api->pcm_sched_type_is_async = pcm_sched_type_is_async;
        ...
        start_lgs(goal_filename, *lgs);  // lgs
