 ...
 */
int state_machine(struct ccp_connection *conn, struct timespec *start) {
...
#ifdef PROFILE_DP_PROGRAM
    struct timespec end;
    clock_gettime(CLOCK_MONOTONIC, &end);
    u64 diff = (u64)(end.tv_sec - start->tv_sec) * 1000000000 + (end.tv_nsec - start->tv_nsec);
    printf("[PROFILE_DP_PROGRAM]: %llu ns\n", (unsigned long long)diff);
#endif
#ifdef PROFILE_DP_IPC
    struct timespec ipc_start;
    clock_gettime(CLOCK_MONOTONIC, &ipc_start);
#endif
...
#ifdef PROFILE_DP_IPC
    struct timespec ipc_end;
    clock_gettime(CLOCK_MONOTONIC, &ipc_end);
    u64 ipc_diff = (u64)(ipc_end.tv_sec - ipc_start.tv_sec) * 1000000000 + (ipc_end.tv_nsec - ipc_start.tv_nsec);
    printf("[PROFILE_DP_IPC]: %llu ns\n", (unsigned long long)ipc_diff);
#endif
