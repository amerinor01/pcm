    ...
    if (tm_file != NULL) {
        ...
        while (eventlist.doNextEvent()) {
            ...
            for (std::size_t i = 0; i < pcm_srcs.size(); ++i) {
                num_finished_flows += pcm_srcs[i]->isFlowFinished() ? 1 : 0;
            }
            if (num_finished_flows == pcm_srcs.size()) {
