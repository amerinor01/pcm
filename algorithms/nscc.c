...
static PCM_FORCE_INLINE pcm_uint nscc_get_avg_q_delay(ALGO_CTX_ARGS) {
    return get_var_uint(AVG_Q_DELAY);
}
static PCM_FORCE_INLINE void nscc_update_q_delay(ALGO_CTX_ARGS, bool ecn) {
    pcm_uint new_avg_q_delay;
    pcm_uint q_delay =
        (get_signal(RTT_SAMPLE) > BRTT) ? (get_signal(RTT_SAMPLE) - BRTT) : 0;
    if (!ecn && q_delay > TARGET_Q_DELAY) {
        new_avg_q_delay =
            EWMA_DELAY_ALPHA * BRTT * 0.25 + (1 - EWMA_DELAY_ALPHA) * get_var_uint(AVG_Q_DELAY);
    } else {
        if (q_delay > 5 * BRTT) {
            new_avg_q_delay = 0.0125 * q_delay + (1 - 0.0125) * get_var_uint(AVG_Q_DELAY);
        } else {
            new_avg_q_delay =
                EWMA_DELAY_ALPHA * q_delay + (1 - EWMA_DELAY_ALPHA) * get_var_uint(AVG_Q_DELAY);
        }
    }
    set_var_uint(AVG_Q_DELAY, new_avg_q_delay);
}

static PCM_FORCE_INLINE void nscc_fair_increase(ALGO_CTX_ARGS) {
    pcm_float increment = FI * get_signal(NUM_ACKED_BYTES);
    set_var_uint(FULFILL_ADJ_INCREMENT, get_var_uint(FULFILL_ADJ_INCREMENT) + increment);
}
...
static PCM_FORCE_INLINE void nscc_fast_increase(ALGO_CTX_ARGS, pcm_uint q_delay,
    ...
    if (q_delay < FI_QDELAY_TOL) {
        set_var_uint(FAST_COUNT,
                     get_var_uint(FAST_COUNT) + get_signal(NUM_ACKED_BYTES));
        if (get_var_uint(FAST_COUNT) > *cur_cwnd || get_var_uint(FAST_ACTIVE)) {
            *cur_cwnd += get_signal(NUM_ACKED_BYTES) * FI_SCALE;
...
static PCM_FORCE_INLINE void nscc_proportional_increase(ALGO_CTX_ARGS, pcm_uint q_delay,
    ...
    if (get_var_uint(FAST_ACTIVE))
        return;

    pcm_float cwnd_increment =
        PI_ALPHA * (pcm_float)(get_signal(NUM_ACKED_BYTES) * (TARGET_Q_DELAY - q_delay));
    set_var_uint(FULFILL_ADJ_INCREMENT,
                 get_var_uint(FULFILL_ADJ_INCREMENT) + cwnd_increment);
}
...
static PCM_FORCE_INLINE void nscc_multiplicative_decrease(ALGO_CTX_ARGS, pcm_uint *cur_cwnd) {
    ...
    if (nscc_get_avg_q_delay(ALGO_CTX_PASS) > TARGET_Q_DELAY) {
        if (get_signal(ELAPSED_TIME) - get_var_uint(T_MD_LAST) > BRTT) {
...
static PCM_FORCE_INLINE void nscc_core_cases(ALGO_CTX_ARGS, pcm_uint q_delay, pcm_uint *cur_cwnd) {
    if (!get_signal(NUM_ECN) && q_delay >= TARGET_Q_DELAY) {
        ...
        //        get_signal(ELAPSED_TIME), *cur_cwnd);
    } else if (!get_signal(NUM_ECN) && q_delay < TARGET_Q_DELAY) {
        ...
        //        *cur_cwnd, get_signal(NUM_ACKED_BYTES), q_delay, TARGET_Q_DELAY,
        //        get_signal(RTT_SAMPLE));
    }
}
...
static PCM_FORCE_INLINE bool nscc_quick_adapt(ALGO_CTX_ARGS, pcm_uint q_delay, bool got_ecn,
                                              ...
                                              pcm_uint *cur_cwnd) {
    set_var_uint(QA_BYTES_IGNORED, get_var_uint(QA_BYTES_IGNORED) + ignore_bytes);

    bool qa_done_or_ignore = false;
    if ((get_var_uint(QA_BYTES_IGNORED) < get_var_uint(QA_BYTES_TO_IGNORE)) && got_ecn) {
        qa_done_or_ignore = true;
    } else if (get_signal(ELAPSED_TIME) >= get_var_uint(QA_DEADLINE)) {
        if (get_var_uint(QA_DEADLINE) != 0 &&
            (get_var_uint(QA_TRIGGER) || is_loss || (q_delay > QA_THRESHOLD)) &&
            get_var_uint(QA_ACKED_BYTES) < (BDP >> QA_GATE)) {
            set_var_uint(QA_TRIGGER, 0);
            set_var_uint(QA_BYTES_TO_IGNORE, get_signal(IN_FLIGHT_BYTES));
            set_var_uint(QA_BYTES_IGNORED, 0);
            *cur_cwnd = (pcm_uint)(MAX(get_var_uint(QA_ACKED_BYTES), (pcm_uint)MSS));
            qa_done_or_ignore = true;
        }
        set_var_uint(QA_ACKED_BYTES, 0);
        set_var_uint(QA_DEADLINE, get_signal(ELAPSED_TIME) + TRTT);
    }

    if (qa_done_or_ignore) {
        set_var_uint(FULFILL_ADJ_INCREMENT, 0);
        set_var_uint(FULFILL_ADJ_BYTES, 0);
    }
...
static PCM_FORCE_INLINE void nscc_handle_ack(ALGO_CTX_ARGS, pcm_uint q_delay, pcm_uint *cur_cwnd) {
    set_var_uint(QA_ACKED_BYTES,
                 get_var_uint(QA_ACKED_BYTES) + get_signal(NUM_ACKED_BYTES));
    set_var_uint(FULFILL_ADJ_BYTES,
                 get_var_uint(FULFILL_ADJ_BYTES) + get_signal(NUM_ACKED_BYTES));

    if (nscc_quick_adapt(ALGO_CTX_PASS, q_delay, get_signal(NUM_ECN), false,
                         get_signal(NUM_ACKED_BYTES), cur_cwnd)) {
...
static PCM_FORCE_INLINE void nscc_handle_loss_signal(ALGO_CTX_ARGS, pcm_uint *cur_cwnd) {
    set_var_uint(QA_TRIGGER, 1);

    if (!nscc_quick_adapt(ALGO_CTX_PASS, 0, true, true, get_signal(NUM_NACKED_BYTES),
                          cur_cwnd)) { // && (!_receiver_based_cc || !last_hop)) {
        pcm_uint nacked = get_signal(NUM_NACKED_BYTES);
        if (*cur_cwnd <= nacked) {
            *cur_cwnd = MSS;
        } else {
...
static PCM_FORCE_INLINE void nscc_fulfill_adjustment(ALGO_CTX_ARGS, pcm_uint *cur_cwnd) {
    if ((get_var_uint(FULFILL_ADJ_BYTES) > FULFILL_ADJ_BYTES_THRESH) ||
        (get_signal(ELAPSED_TIME) - get_var_uint(LAST_FULFILL_ADJ_TIME)) >
            FULFILL_ADJ_TIME_THRESH) {
        *cur_cwnd += get_var_uint(FULFILL_ADJ_INCREMENT) / *cur_cwnd;
        if ((get_signal(ELAPSED_TIME) - get_var_uint(LAST_FULFILL_ADJ_TIME)) >
            FULFILL_ADJ_TIME_THRESH) {
            *cur_cwnd += (pcm_uint)ETA;
            set_var_uint(LAST_FULFILL_ADJ_TIME, get_signal(ELAPSED_TIME));
        }
        set_var_uint(FULFILL_ADJ_BYTES, 0);
        set_var_uint(FULFILL_ADJ_INCREMENT, 0);
...
int algorithm_main() {
    pcm_uint q_delay;
    if (get_signal(RTT_SAMPLE) >= BRTT) {
        nscc_update_q_delay(ALGO_CTX_PASS, get_signal(NUM_ECN));
        q_delay = get_signal(RTT_SAMPLE) - BRTT;
    } else {
