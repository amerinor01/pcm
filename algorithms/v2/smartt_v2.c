...
static PCM_FORCE_INLINE bool smartt_quick_adapt(ALGO_CTX_ARGS, pcm_uint t_now, pcm_uint *cur_cwnd) {
    ...
    if (t_now >= get_var_uint(QA_DEADLINE)) {
        if (get_var_uint(QA_DEADLINE) != 0 && get_var_uint(TRIGGER_QA)) {
            ...
            adapted = true;
            set_var_uint(BYTES_TO_IGNORE, *cur_cwnd);
...
static PCM_FORCE_INLINE bool smartt_fast_increase(ALGO_CTX_ARGS, pcm_uint num_ecns, pcm_uint rtt_sample, pcm_uint *cur_cwnd) {
    if ((ABS((pcm_float)rtt_sample - BRTT) < (FI_BRTT_TOL * (pcm_float)BRTT)) && !num_ecns) {
        set_var_uint(FAST_COUNT, get_var_uint(FAST_COUNT) + MSS);
...
static PCM_FORCE_INLINE void smartt_handle_ack(ALGO_CTX_ARGS, pcm_uint num_ecns, pcm_uint rtt_sample, pcm_uint t_now, pcm_uint *cur_cwnd) {
    ...
    if (get_var_uint(BYTES_IGNORED) < get_var_uint(BYTES_TO_IGNORE)) {
        set_var_uint(BYTES_IGNORED, get_var_uint(BYTES_IGNORED) + MSS);
...
int algorithm_main() {
    pcm_uint rtt_sample = get_signal(RTT_SAMPLE);
    pcm_uint t_now = get_signal(ELAPSED_TIME);
    pcm_uint cur_cwnd = get_control(CWND_BYTES);

    // Calculate Deltas
    pcm_uint num_nack = get_signal(NUM_NACK) - get_var_uint(PREV_NACK);
    pcm_uint num_rto = get_signal(NUM_RTO) - get_var_uint(PREV_RTO);
    pcm_uint num_ack = get_signal(NUM_ACK) - get_var_uint(PREV_ACK);
    pcm_uint num_ecns = get_signal(NUM_ECN) - get_var_uint(PREV_ECN);

    // Update Cache
    set_var_uint(PREV_NACK, get_signal(NUM_NACK));
    set_var_uint(PREV_RTO, get_signal(NUM_RTO));
