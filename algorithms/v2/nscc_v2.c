...
static PCM_FORCE_INLINE pcm_uint nscc_get_avg_q_delay(ALGO_CTX_ARGS) { return get_var_uint(AVG_Q_DELAY); }
static PCM_FORCE_INLINE void nscc_update_q_delay(ALGO_CTX_ARGS, bool ecn) {
    pcm_uint new_avg_q_delay;
    pcm_uint q_delay = (get_signal(RTT_SAMPLE) > BRTT) ? (get_signal(RTT_SAMPLE) - BRTT) : 0;
    if (!ecn && q_delay > TARGET_Q_DELAY) {
        new_avg_q_delay = EWMA_DELAY_ALPHA * BRTT * 0.25 + (1 - EWMA_DELAY_ALPHA) * get_var_uint(AVG_Q_DELAY);
    } else {
        if (q_delay > 5 * BRTT) {
            new_avg_q_delay = 0.0125 * q_delay + (1 - 0.0125) * get_var_uint(AVG_Q_DELAY);
        } else {
            new_avg_q_delay = EWMA_DELAY_ALPHA * q_delay + (1 - EWMA_DELAY_ALPHA) * get_var_uint(AVG_Q_DELAY);
        }
    }
...
static PCM_FORCE_INLINE void nscc_fast_increase(ALGO_CTX_ARGS, pcm_uint q_delay, pcm_uint *cur_cwnd, pcm_uint newly_acked_bytes) {
    if (q_delay < FI_QDELAY_TOL) {
        set_var_uint(FAST_COUNT, get_var_uint(FAST_COUNT) + newly_acked_bytes);
...
static PCM_FORCE_INLINE void nscc_proportional_increase(ALGO_CTX_ARGS, pcm_uint q_delay, pcm_uint *cur_cwnd, pcm_uint newly_acked_bytes) {
    ...
    set_var_uint(FULFILL_ADJ_INCREMENT, get_var_uint(FULFILL_ADJ_INCREMENT) + cwnd_increment);
}
...
static PCM_FORCE_INLINE bool nscc_quick_adapt(ALGO_CTX_ARGS, pcm_uint q_delay, bool got_ecn, bool is_loss, pcm_uint ignore_bytes,
    ...
    if ((get_var_uint(QA_BYTES_IGNORED) < get_var_uint(QA_BYTES_TO_IGNORE)) && got_ecn) {
        ...
        if (get_var_uint(QA_DEADLINE) != 0 && (get_var_uint(QA_TRIGGER) || is_loss || (q_delay > QA_THRESHOLD)) &&
...
static PCM_FORCE_INLINE void nscc_handle_ack(ALGO_CTX_ARGS, pcm_uint q_delay, pcm_uint *cur_cwnd, pcm_uint delta_acked_bytes,
                                             pcm_uint delta_ecn) {
    set_var_uint(QA_ACKED_BYTES, get_var_uint(QA_ACKED_BYTES) + delta_acked_bytes);
    set_var_uint(FULFILL_ADJ_BYTES, get_var_uint(FULFILL_ADJ_BYTES) + delta_acked_bytes);
...
static PCM_FORCE_INLINE void nscc_fulfill_adjustment(ALGO_CTX_ARGS, pcm_uint *cur_cwnd) {
    if ((get_var_uint(FULFILL_ADJ_BYTES) > FULFILL_ADJ_BYTES_THRESH) ||
        (get_signal(ELAPSED_TIME) - get_var_uint(LAST_FULFILL_ADJ_TIME)) > FULFILL_ADJ_TIME_THRESH) {
        *cur_cwnd += get_var_uint(FULFILL_ADJ_INCREMENT) / *cur_cwnd;
        if ((get_signal(ELAPSED_TIME) - get_var_uint(LAST_FULFILL_ADJ_TIME)) > FULFILL_ADJ_TIME_THRESH) {
...
int algorithm_main() {
    pcm_uint q_delay;
    // Calculate Deltas
    pcm_uint delta_ecn = get_signal(NUM_ECN) - get_var_uint(PREV_ECN);
    pcm_uint delta_nacked_bytes = get_signal(NUM_NACKED_BYTES) - get_var_uint(PREV_NACKED_BYTES);
    pcm_uint delta_acked_bytes = get_signal(NUM_ACKED_BYTES) - get_var_uint(PREV_ACKED_BYTES);

    // Update Cache
    set_var_uint(PREV_ECN, get_signal(NUM_ECN));
    set_var_uint(PREV_NACKED_BYTES, get_signal(NUM_NACKED_BYTES));
    set_var_uint(PREV_ACKED_BYTES, get_signal(NUM_ACKED_BYTES));
