...
int algorithm_main() {
    ...
    if (trigger_mask & NUM_NACKED_BYTES) {
        ...
        if (cur_cwnd > get_signal(NUM_NACKED_BYTES))
            cur_cwnd = MAX((pcm_uint)MSS, get_control(CWND_BYTES) - get_signal(NUM_NACKED_BYTES));
        ...
        pcm_uint cur_cwnd = get_control(CWND_BYTES);
        pcm_uint newly_acked_bytes = get_signal(NUM_ACKED_BYTES);
