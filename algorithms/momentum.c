...
int algorithm_main() {
    pcm_uint initialized = get_var_uint(VAR_INIT);
    if (!initialized) {
        printf("MOMEMTUM: initializing handler\n");
        set_var_uint(VAR_LAST_ACK_TRIGGERED_TIME, get_signal(SIG_TIME_STAMP)); // for algorithm v1
        set_var_uint(VAR_INIT, 1);
    }
    ...
    if (trigger_mask & SIG_NUM_NACK) {
        ...
        printf("MOMEMTUM: curr_cwnd=%llu bytes_sent=%llu num_ecn=%llu num_ack=%llu ",
               cur_cwnd, get_signal(SIG_BYTES_SENT), get_signal(SIG_NUM_ECN), get_signal(SIG_NUM_ACK));
        ...
        pcm_uint bytes_sent = get_signal(SIG_BYTES_SENT);
        ...
        if (!ALGO_V2) {
            ...
            printf(" acked_bytes=%llu dw(exp)=%f dw(lin_dec)=%f dw(lin_recov)=%f total_dw=%lld\n",
                   ...
                   dw);
            set_signal(SIG_NUM_ACKED_BYTES, 0);
        }
        ...
        set_control(CTRL_CWND_BYTES, MIN(MAX(cur_cwnd, MINIMUM_CWND_BYTES), MAXIMUM_CWND_BYTES));
